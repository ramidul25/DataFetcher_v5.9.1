"""
************************************************************
*  DataFetcher - v5.9.1  (Will Expire on 31-12-2027)       *
...
expiry_date = datetime(2027, 12, 31)  # Set your expiry date here
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print()
        print(colored("1. Device Configuration Backup", "green"))
        print(colored("2. Pre / Post Check of Planned Outage", "green"))
        print(colored("3. Pre / Post Validation of BT-PEW ", "green"))
        print(colored("\nq. Quit", "red"))
        ...
        if choice == 'q':
            ...
            commands = ["sh int desc | i WAN", "sh ip int bri | i 0/1/0", "sh bgp all su | b N", "sh clock"]

            print(colored("\nYou are about to run the following commands (RA/RB only):", "cyan"))
            for cmd in commands:
                print(f"- {cmd}")
            confirm = input(colored("Proceed? (y/n): ", "yellow")).strip().lower()
            if confirm != 'y':
                continue
            ...
            success_flag = False
            with ThreadPoolExecutor(max_workers=len(site_numbers) * 2) as executor:  # 2 devices (RA & RB) per site
                futures = {}
                for site in site_numbers:
                    site_folder = os.path.join(main_folder, f"{site}")
                    os.makedirs(site_folder, exist_ok=True)

                    router_hosts = [
                        f"ra.s0{site}.asda.uk",
                        f"rb.s0{site}.asda.uk"
                    ]
                    for host in router_hosts:
                        futures[executor.submit(
                            ssh_to_router, host, ssh_port, username, password, commands, site_folder
                        )] = host
            ...
            if success_flag:
                print("\n✅ Task executed successfully on the reachable RA/RB devices.")
                input("\nPress Enter to return to the main menu...")
            else:
                print("\n❌ No RA/RB devices were reachable. Please try again.")
                input("\nPress Enter to return to the main menu...")
            continue
            ...
            continue
        # Shared logic for options 1 & 2
        print(colored("\nYou are about to run the following commands:", "cyan"))
        for cmd in commands:
            print(f"- {cmd}")
        confirm = input(colored("Proceed? (y/n): ", "yellow")).strip().lower()
        if confirm != 'y':
